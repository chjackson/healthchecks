...
# This script doesn't do separate model runs with treated subsets
# Use this for rougher results with no comp uncertainty
...
def SaveResults(M, S, N, P, ST, nsc, npops, nouts, prefix):
    ...
    np.savetxt("results/%s_process.csv" % prefix, P, delimiter=",") #
    np.savetxt("results/%s_short.csv" % prefix, ST.reshape((nsc*16,41)), fmt="%s", delimiter=",") #
...
for i in range(comb):
    ...
    H1.Run()
    M[r,],S[r,],N[r,],P[r,],ST[r,] = gr.GetResults_all(H, H1, M[r,], S[r,], N[r,], P[r,], ST[r,])
    SaveResults(M, S, N, P, ST, nsc, npops, nouts, prefix)
    r += 1
