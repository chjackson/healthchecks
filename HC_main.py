...
#            
            ...
            if self.Health_Checks == True:
                ...
                current_turnup = self.Turnup.copy()
                red_turnup_factor = 0.01
                if i == 1:
                    # for year 1 in simulation, if HC was last year
                    current_turnup[self.Attending[:,0] == 1] *= red_turnup_factor
                if i == 2:
                    # for year 2 in simulation, if HC was last year
                    current_turnup[self.Attending[:,1] == 1] *= red_turnup_factor            
                if i == 3:
                    # for year 3 in simulation, if HC was last year
                    current_turnup[self.Attending[:,2] == 1] *= red_turnup_factor
                if i == 4:
                    # for year 4 in simulation, if HC was last year
                    current_turnup[self.Attending[:,3] == 1] *= red_turnup_factor
                if i > 4:
                    # for all other years in simulation, if HC was last 4 year
                    current_turnup[self.Attending[:,i-4:i].sum(axis=1) >= 1] *= red_turnup_factor                    
                
                self.current_turnup[:,i] = current_turnup * self.alive[:,i]
                
                turnup = self.eligible[:, i] * current_turnup
